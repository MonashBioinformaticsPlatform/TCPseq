...
for i in $(seq 0 $FNUM); do
  ...
  STAR --outSAMunmapped Within --readFilesCommand zcat --genomeDir $RRI_S --runThreadN $NCORES --outFilterMultimapNmax 20000  \
    --readFilesIn ../trimmed/m14_fpc_pA_tr_$bn.fq.gz --outFileNamePrefix fpc_vs_rR_$bn  2> fpc_vs_rR_$bn.txt
  # get unmapped reads
  ...
  STAR --outSAMunmapped Within --readFilesCommand zcat --genomeDir $SRI_S --runThreadN $NCORES --outFilterMultimapNmax 20000  \
    --readFilesIn norRtR_$bn.fq.gz --outFileNamePrefix RTS_$bn  2>miscmap_star_$bn.txt
